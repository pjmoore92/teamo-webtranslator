...
website is support PayPal for payment of translated document. To
...
provides the translator with a feature that will not only guarantee her
security but also will encourage her customers to use the site. PayPal's
incredible share of the online payments market encourgaes a feeling of reassurance
during the payment stage. The translator, therefore, does not need to be concerned
with things such as credit card verification. All of this functionality is
...
website's implementation, we emailed her without hesitation and she was quite often happy to
...
automatically. From this we can surmise there is a large audience withint the
...
Subsequently, in Section 2 the user can enteri their contact information, add
...
file store, before placin the job in
...
This section aim to show the user interface of our final product. 
From the structure, the navigation to the description of all main pages.
...
metatags to increase findability via a search engine. A metatag is extra information contained within the HTML
...
\end{lstlisting}
With these metatags in place, it helps search engines match pages to queries such that if a user entered ``english to
french translation'', it should increase the page ranking of that relevant page for BethelTranslations. Including
metatags to improve SEO is not as magical as it may seem though. The best way of increasing page visibility on 
...
research discussed in the \ref{sect:reliab} section later. Therefore, over time, the website can and should become
more popular.
\section{Security}
...
\small{\textit{www.krystal.co.uk}}. In brief, it offers afforable, competitve plans and is situated in a 
``bunker datacentre''. We were able to find an excellent plan for the translator that involved,a mong other
...
to take frequent backups of data, excellent phsyical security and round the clock customer support. We were
...
behaviour, neither can they recognize every error in the source code. Carefully
...
session. Most notably, the PayPal stage was a block to the particpants
completing the task's asked of them. Due to an incorectly set API key,
...
web development techniques, and we were worried about not meeting the clients requirements. The This ensured that the project tested each team member’s intelligence
and desire to learn. The following section revises the main aims, summarises the achievements and
what was learned, and then suggests possible future work in the area.
...
by the general public. The only thing that currently needs to be fixed is to change the payment action to a real PayPal account instead of
the testing sandbox.
...
about whether or not to use Bethel Translations and then to upload their documents for quotation.
...
we will try to address any issues that may arise. 
