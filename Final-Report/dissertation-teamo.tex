...
is analysing the layout of that particular page, rather than the visual design.
...
each screen in [Project name].
Unless otherwise noted, no graphic treatment has yet occurred.

\textbf{Welcome}
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{images/welcomePage}
\vspace{-30pt}
\end{figure}
\begin{itemize}
\item \textbf{Description}

The welcome page first provides navigation links to every other pages. It shows the welcome information from the translator.
The main functionality of this page may be allowing clients to log in or regester here. 

We decided to make the registration process as a three step process so that making it as simple as possible for a first time user.
Also we avoid some spam by this as an actual job submittion is required in order to register.

\item \textbf{Navigation}

This is the default page a client will see when visiting the website. One can always come back to this page with the text link "Home".
However, the registration process will not be shown for a logged-in user. Instead, he may submit a new job through the link in the dashboard.

\item \textbf{Main Elements}

The following is a list of all elements in this screen.
[Some sample elements included here. Use a pro-forma to ensure
consistent treatment for all elements in any set.]
‘Rail Line’
Type: Dropdown list box
Label: ‘Rail Line’
Content: List of all rail lines, sorted alphabetically, preceded by
        the default.
Default: ‘Choose from this list’ if the user has accessed the
            page from an external link, or without previously
           choosing a rail line.
A specific rail line if the user has chosen one on a
previous screen.
Behaviour: Standard.
Comment: None.
...
\begin{center}
    ...
    Description & As a prerequisite-requisite to requirements gathering we must research the current trends in web-development and research what works and doesn't work in a user friendly website. \\ \hline   
...
\label{sect:org}
At the beginning of the project the team were uncertain as to who was most skilled in specific areas so it was decided that everyone would be involved everywhere, to varying levels. As the project progreses it will become clear who is a strong leader in specific areas and those members will then be able to lead the way for that part of the project.
...
\begin{center}
    ...
    Personnel shortfall at crucial development stages & Low & High & Illness, other commitments  \\ \hline
    Unrealistic schedule not adhered to & Medium & Medium & Improper planning of work time line \\ \hline
    Inaccurate requirements capture & Medium & High & Miscommunication with client \\ \hline
    Lack of technical knowledge in team (PHP, HTML, CSS, JavaScript) & Low & Critical & Lack of commitment
    to other Uni courses or weakness in understanding \\ \hline
    Shortfall in externally performed tests & Low & Low & Laziness, misguided belief that software is fully secure \\ \hline
    Team Members external commitments & Medium & Medium & Team member has commitments that prevent them carrying out team tasks \\ \hline
    Requirement volatility - Client demands change & Medium & High & Client has change of circumstances e.g. budget, time, mind-set \\ 
    \hline
    \end{tabular}
\end{center}
...
%==============================================================================
% Specified as required subsidary information
